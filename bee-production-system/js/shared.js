...
function applySidebarCollapse(){
  ...
  if(!sidebar) return;
  const collapsed = localStorage.getItem('beeSidebarCollapsed') === '1';
...
function renderSidebar(){
  ...
  // Sidebar collapse toggle — .onclick assignment (not addEventListener) so
  // re-running renderSidebar() on every page render doesn't stack listeners.
  const collapseBtn = document.getElementById('sidebarCollapseBtn');
  if(collapseBtn){
    collapseBtn.onclick = ()=>{
      const isCollapsed = document.getElementById('sidebar').classList.toggle('collapsed');
      localStorage.setItem('beeSidebarCollapsed', isCollapsed ? '1' : '0');
    };
  }
