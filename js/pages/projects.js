  async function loadProjectAssets(projectId){
    try {
      const response = await fetch(`../api/assets.php?project_id=${projectId}`);
      const data = await response.json();

      if(!data.success){
        console.error('Assets API Error:', data);
        return;
      }

      DB.assets = data.assets.map(a => ({
        id: String(a.id),
        project: String(a.project_id),
        title: a.title,
        type: a.type,
        link: a.link,
  ...
  function pageProjectDetail(){
    ...
    ${assets.length ? assets.map(a => {
      ...
      return `
        <div class="list-row">
          <div class="type-tag" style="background:${meta.color}22;color:${meta.color};">
            ${esc(meta.tag)}
          </div>
          ...
          <div style="flex:1;">
            ...
            <div class="row-sub">
              ${esc(a.type || 'Asset')} · created ${fmtDate(a.created_at)}
            </div>
          </div>
