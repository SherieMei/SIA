...
function load_state($pdo) {
    $r = $pdo->query('SELECT state_json FROM app_state WHERE id=1')->fetch();
    $s = $r ? json_decode($r['state_json'], true) : [];
    return is_array($s) ? $s : [];
}

function save_state($pdo, $s) {
    $s['currentUser'] = null;
    $j = json_encode($s, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    $q = $pdo->prepare('INSERT INTO app_state (id, state_json) VALUES (1, ?) ON DUPLICATE KEY UPDATE state_json=VALUES(state_json)');
    $q->execute([$j]);
}
